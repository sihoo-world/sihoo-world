# 👋 안녕하세요, 로우레벨 하드웨어 및 에지 AI 엔지니어 유시후입니다.

제한된 리소스 환경에서의 임베디드 최적화, 다층 PCB 설계, 그리고 TinyML 경량화에 관심이 많은 개발자입니다.

---

## 🛠️ 기술 스택 (Tech Stacks)
* **Languages:** Rust, C, Python
* **Hardware/EDA:** EasyEDA Pro, 고속 디지털 회로 설계 및 다층 PCB 라우팅 (최대 6-Layer)
* **Embedded/AI:** TinyML, 에지 AI 모델 최적화, 정밀 센서 데이터 수집 및 튜닝

![Rust](https://img.shields.io/badge/Rust-000000?style=flat&logo=rust&logoColor=white)
![C](https://img.shields.io/badge/C-A8B9CC?style=flat&logo=c&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![EasyEDA](https://img.shields.io/badge/EasyEDA-006699?style=flat&logo=easyeda&logoColor=white)
![PCB Design](https://img.shields.io/badge/PCB_Design-8C8C8C?style=flat&logo=adafruit&logoColor=white)
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=flat&logo=arduino&logoColor=white)

---

## 📌 주요 연구 및 프로젝트 (Projects)

### 🛡️ 에지 AI 기반 임베디드 시스템 연구 (2026 SDC 대회 제출작)
* **개요:** 아두이노 나노의 제한된 환경 이내에서 TinyML을 통한 인근 환경 정보 측정 임베디드 디바이스 연구
* **담당:** MCU 환경 구동을 위한 TinyML 모델 양자화(Quantization) 및 행렬 연산 최적화, 95% 이상 추론 정확도 달성
* **결론:** 특선 입상

### 🚒 저성능 임베디드 보드 시스템 기반 연구 (소방청 논문 대회 제출 예정)
* **개요:** 극한 환경에서 소방대원의 생체 신호 및 이상 징후를 실시간 추론하는 임베디드 디바이스 연구
* **담당:** 저전력, 저성능 환경에서 구동을 위한 최적화 및 검증
* **결론:** ~ing

---

## 🔗 식별자 및 자격 스펙 (Identifiers & Certifications)
* **ORCID ID:** [0009-0005-2866-6511]
* **보유 자격:** 무인동력비행장치 4종(무인멀티콥터) / 무인동력비행장치 4종(무인비행기) / 무인동력비행장치 4종(무인헬리콥터) / 무인동력비행장치 4종(무인수직이착륙기)

---

[0009-0005-2866-6511]: https://orcid.org/0009-0005-2866-6511
